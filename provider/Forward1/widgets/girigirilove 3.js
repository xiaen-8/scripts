...
async function scanCategoryForTitle(wantBaseNorm, isMovie) {
  // Scan category pages (日番) for best matching title
  var maxPages = 1;
  ...
  for (var ci = 0; ci < catIds.length && bestScore < 300; ci++) {
    ...
    for (var pg = 1; pg <= maxPages; pg++) {
      ...
      pageReqs.push(Widget.http.get(url, { headers: HEADERS }).then(function(r) {
        return (r && r.data) || "";
      }).catch(function() { return ""; }));
    }
    // Fetch pages in parallel within the same category
...
async function loadResource(params) {
  try {
    ...
    // 1. Site search is always blocked by Cloudflare — skip straight to category scan
...
async function loadDetail(link) {
  try {
    ...
    $scGroup.find("ul.anthology-list-play li a.hide.this-link").each(function () {
      var href = $(this).attr("href") || "";
      var epNum = $(this).text().trim();
      if (href && epNum) {
        var epId = href.replace(/^\//, "").replace(/\/$/, "");
        episodeItems.push({
          id: epId,
          type: "url",
          title: "第" + parseInt(epNum) + "集",
          link: epId,
        });
      }
    });
